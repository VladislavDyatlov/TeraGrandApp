...
export const Post = ({ color }) => {
  ...
  const addPost = async (title, img, comment, description) => {
    addDoc(collection(db, "Posts"), {
      ...
      .then(() => {
        alert.show("Ваши данные успешно отправлены", "success");
      })
      .catch(() => {
        alert.show("Что-то пошло не так. Возможна ошибка сервера", "error");
      });
  };
  ...
  return (
    <>
      <div className={`post ${color ? "active" : ""}`}>
        <div className="post__container">
          ...
          <div>
            <Button
              variant="contained"
              color="success"
              onClick={() => addPost(title, img, comment, description)}
            >
              Отправить
            </Button>
          </div>
